...
class CustomViewModulefields extends ViewModulefields
{
	function display()
	{
        ...
        $studioPlusFields = array();
        $smarty->assign('current_module', $module_name);
        ...
        if(!isset($_REQUEST['view_package']) || $_REQUEST['view_package'] == 'studio') {
            ...
            foreach($dictionary[$objectName]['fields'] as $def) {
                ...
                if(isset($def['icc_is_studio_plus'])) {
                    $studioPlusFields[$def['name']] = true;
                }
            ...
            $smarty->assign('studioPlusFields', getJSONobj()->encode($studioPlusFields));
            ...
            $ajax->addSection('center', $mod_strings['LBL_EDIT_FIELDS'],$smarty->fetch('custom/modules/ModuleBuilder/tpls/fields.tpl'));
            $_REQUEST['field'] = '';
