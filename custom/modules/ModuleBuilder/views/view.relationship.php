...
class CustomViewRelationship extends SugarView
{
    /**
    ...
    public function display()
    {
        ...
        if (!empty($relationshipName) && isset($relationships->getRelationshipList()[$relationshipName])) {
        ...
        $iccRelationshipDefs = $relationship->getDefinition();

        if($relationshipName) {
            $iccRelationshipDefs['icc_lhs_link_name'] = array_reduce($GLOBALS['dictionary'][$GLOBALS['beanList'][$relationship->lhs_module]]['fields'], function($carry, $field) use ($relationshipName) {
                if(isset($field['relationship']) && $field['relationship'] === $relationshipName) {
                    return $field['name'];
                }
            ...
            $iccRelationshipDefs['icc_rhs_link_name'] = array_reduce($GLOBALS['dictionary'][$GLOBALS['beanList'][$relationship->rhs_module]]['fields'], function($carry, $field) use ($relationshipName) {
                if(isset($field['relationship']) && $field['relationship'] === $relationshipName) {
                    return $field['name'];
                }
            ...
            $iccRelationshipDefs['icc_rhs_link_name'] = 'icc_' . str_replace('icc_', '', strtolower($iccRelationshipDefs['lhs_module']));
            $iccRelationshipDefs['icc_lhs_link_name'] = 'icc_' . str_replace('icc_', '', strtolower($iccRelationshipDefs['rhs_module']));
        }
        ...
        $this->smarty->assign('rel', $iccRelationshipDefs);
        ...
        if (isset($_REQUEST['json']) && $_REQUEST[ 'json'] == 'false') {
            echo $this->smarty->fetch('custom/modules/ModuleBuilder/tpls/studioRelationship.tpl');
        } else {
